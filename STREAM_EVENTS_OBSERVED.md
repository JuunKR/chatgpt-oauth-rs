...
`examples/dump_events.rs` 로 실제 `chatgpt.com/backend-api/codex/responses` 에 요청을 보내
...
```bash
cargo run --example dump_events -- "1+1은? 숫자만 한 글자로."          # 일반 텍스트
cargo run --example dump_events -- --tool "서울 날씨 알려줘. get_weather 써."  # 툴 콜
```
...
## 2.5) 서버 빌트인 툴 (web_search) — function_call 과 다른 패밀리

`SendOptions.tools = [{"type":"web_search"}]` 로 활성화(opt-in). 재현:
`cargo run --example dump_events -- --web "...웹 검색 유도 프롬프트..."`.
...
### 빌트인 툴의 존재 흔적 (활성화 안 해도)
모든 응답의 `response` 에코에 `tool_usage` 회계 블록이 항상 있다:
```json
"tool_usage": {
...
```
→ 서버가 인지하는 빌트인 툴 카테고리: **`image_gen`(이미지 생성), `web_search`**.
안 켜면 `num_requests:0` / 토큰 0. (image_gen 은 아직 미검증 — `{"type":"image_generation"}`
로 추후 덤프 필요.)
...
response.function_call_arguments.delta     ← 툴 콜일 때
response.function_call_arguments.done       ← 툴 콜일 때
response.completed
```
...
## 4) 이 크레이트의 현재 노출 방식 & 개선 메모

- `send_message` → 위 이벤트를 전부 소비해 **최종 `response`(output 합성 포함) 하나**를 반환.
- `open_stream` / `open_stream_with_input` → **원본 `serde_json::Value` 이벤트**를 그대로 스트림.
  → 소비자가 `"response.output_text.delta"`, `"function_call"`, `item.arguments` 같은
    **매직 문자열/구조를 직접 알아야** 함. 타입 없음 → 발견성 낮고 OpenAI 변경에 취약.

**개선 후보(미결정, 별도 논의):** 타입 이벤트 enum(`StreamEvent::{TextDelta, ToolCall, Completed, Failed, Incomplete, Other}`) 으로 매직 문자열을 크레이트 안에 가두고, 툴 콜은
`ToolCall { call_id, name, arguments }` 로 구조화. 멀티 에이전트엔 텍스트뿐 아니라 툴 콜이
필수라 텍스트 전용 헬퍼만으론 부족.
...
| `responses` (+`--tool`/`--web`) | `POST /responses` (SSE) | ⚠️ 토큰/쿼터 소모. `events:[...]` 원본 |
...
cargo run --example capture --features capture -- responses --web "최신 뉴스 검색"
cargo run --example capture --features capture -- device-code        > device.json
```

### `examples/dump_events.rs` — `/responses` 전용 단일 JSON 덤프
`{meta, events:[...]}` 형태. `--tool`(function) / `--web`(빌트인 web_search) 플래그.
(capture 의 `responses` 시나리오와 겹침 — `/responses` 만 빠르게 볼 때 feature 없이 쓰는 용도.)

```bash
cargo run --example dump_events -- "안녕" > capture-$(date +%Y%m%d).json
jq -r '.events[].type' capture-*.json                                            # 이벤트 타입
jq '.events[0].response.tool_usage' capture-*.json                               # 빌트인 툴 회계
jq '.events[] | select(.type=="response.completed") | .response.usage' capture-*.json  # 토큰
```
